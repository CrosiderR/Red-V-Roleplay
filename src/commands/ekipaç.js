const { SlashCommandBuilder, messageLink,ChannelType, PermissionFlagsBits, GuildMember } = require('discord.js');
...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        try {
             interaction.reply({content: `**Ekip Oluşturma Talebin Başarıyla Oluşturuldu!**`, ephemeral: true});

            const role = await guild.roles.create({ name: Ekipİsmi, color: colorValue });
            await role.setPermissions([]);
            await role.setHoist(true)
            await role.setMentionable(true)
            const ekiplogu = "1173547816572948510"
            ...
            .setColor(renk)
            .setDescription(`> \`${role.name}\` **İsimli Rol Başarıyla Oluşturuldu. Ve Aile Oluşum Sırasına Taşındı.**\n\n> **Oluşturan Kişi: ${interaction.member} / ${interaction.member.id}**\n\n> **Ayarlanan Renk: ${role} / ${roleColor}**`)
            .setTimestamp()

            await client.channels.cache.get(ekiplogu).send({embeds: [embed], content: `@everyone`})
            await interaction.guild.channels.create({
                type: ChannelType.GuildText,
                name: `${role.name}`,
                topic: `${interaction.user.id}`,
                parent: `1171473865814323340`,
                permissionOverwrites: [
                    {   
                        id: interaction.guild.id,
                        deny: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.AttachFiles, PermissionsBitField.Flags.SendMessages]
                    },
                    {
                        id: yetkiliekibi,
                ...
                .setDescription(`${interaction.member} **/** ${interaction.member.id} Tarafından Oluşturuldu! Lütfen Bekleme Kalın!`)
                .setFooter({ text: `${interaction.guild.name}`, iconURL: `${sunucuiconurl}`})
                c.send({embeds: [plainte], content: `${role} **/** <@&${yetkiliekibi}>`})
            }) 
