const { Client,Events, Collection,PermissionsBitField, ChannelType ,GatewayIntentBits, AttachmentBuilder , ButtonBuilder, ButtonStyle, Partials,StringSelectMenuBuilder ,EmbedBuilder, ActionRow, ActionRowBuilder, ContextMenuCommandBuilder, SystemChannelFlagsBitField, ModalBuilder, TextInputBuilder } = require("discord.js");
...
mongoose.connect('mongodb+srv://crosiderm:crosider123@cluster0.5ovqlka.mongodb.net/?retryWrites=true&w=majority', {
...
//Ticket System 9 0 4's Development
								...
								if(interaction.customId === "del") {
									...
									if (interaction.values[0] == "delete2") {
										if(interaction.member.roles.cache.has(yetkiliekibi)) return interaction.reply({content: `**Sen Yetkilisin, Bırak Adam Kapatsın :)**`, ephemeral: true})
								...
								if (interaction.customId == "select") {
									...
									if (interaction.values[0] == "other") {
										await interaction.guild.channels.create({
											...
											.setFooter({ text: `${interaction.guild.name}`, iconURL: `${sunucuiconurl}`})
											c.send({embeds: [partenariat], content: `<@&${yetkiliekibi}> | ${interaction.user}`, components: [row]})
										...
										await interaction.guild.channels.create({
											...
											.setDescription('Yaşadığınız Sorunu Anlatır Mısınız?, Resim, Video Atabilirsiniz.')
											.setFooter({ text: `${interaction.guild.name}`, iconURL: `${sunucuiconurl}`})
											c.send({embeds: [plainte], content: `<@&${yetkiliekibi}> | ${interaction.user}`, components: [row]})	
											interaction.reply({content: `**✔️ Merhaba! Ticket Oluşturma Talebin Başarıyla Alındı! Kanal:** <#${c.id}>`, ephemeral: true})
										})
										...
										await interaction.guild.channels.create({
											...
											.setFooter({ text: `${interaction.guild.name}`, iconURL: `${sunucuiconurl}`})
											c.send({embeds: [embed], content: `<@&${yetkiliekibi}> | ${interaction.user}`, components: [row]})
											interaction.reply({content: `**✔️ Merhaba! Ticket Oluşturma Talebin Başarıyla Alındı! Kanal:** <#${c.id}>`, ephemeral: true})
										})
...
if (süre !== null && (Date.now() - süre) < timeout) {
  ...
  return interaction.reply({ content: `> **Tekrar Yetkililere Bildirim Göndermek İçin Kalan Süre: __${minutes} Dakika__**`, ephemeral: true });
			...
			client.channels.cache.get(sesbildirimtextkanal).send(`${interaction.member} **Adlı Kişi Butona Bastı! Mülakatta Kayıt İçin Sizi Bekliyor. <@&${yetkiliekibi}> <#1163025202658160710>**`)  // Yetkibi Ekibi rolü
...
//destek
	...
	client.on("interactionCreate", async interaction => {
		if (interaction.customId  == "destek1") {
			...
			client.channels.cache.get(sesbildirimtextkanal).send(`${interaction.member} **Adlı Kişi Butona Bastı! Destek Bekleme İçin Sizi Bekliyor. <@&${yetkiliekibi}> <#1163025267275608102>**`)  // Yetkibi Ekibi rolü
	...
	client.on("messageCreate", async message => {
		if (message.content.toLowerCase() === 'v2') {
		   if (!message.member.permissions.has("ADMINISTRATOR")) return
		   const destek1 = new ActionRowBuilder()
		   .addComponents(
			new ButtonBuilder()
			.setCustomId("destek1")
		   ...
		   .setDescription(`● Öncelikle Ufak Sorunlarınız Varsa Destek Bekleme Yerine <#1171474138406330459> Kanalından Ticket Açabilirsiniz.    
		   ...
		   .setImage(`${sunucubanner}`)
		   //message.channel.send({embeds: [embed], components: [ kayıtbutonu1 ]})
		   message.channel.send({content:"||@everyone|| **&** ||@here||", embeds: [embed], components: [ destek1 ]});
		   //.messages.fetch("1011619648900452382").then(msg => { msg.edit({ embeds: [embed] , components: [ kayıtbutonu1 ]})
...
client.on('messageDelete', async message => {
	...
	const logChannel = guild.channels.cache.get('1173550025238908948'); // Silinen mesaj log kanalının ID'sini girin
  
	const botUser = await client.users.fetch("1173550025238908948")
...
try {
	...
	const logChannel2 = guild.channels.cache.get('1173550025238908948'); // Silinen mesaj log kanalının ID'sini girin
...
try {
	...
	.setTitle(`RedV Staff Başvuru Formu`)
	.addComponents(
		...
		if(modal.customId === 'narcos-botlist'){
			...
			**Etiket:** <@${modal.user.id}>
			...
			try {
				...
				.setColor(renk)
				.setAuthor({name: `Never Roleplay`, iconURL: `${sunucubanner}`})
				...
				modal.guild.members.cache.filter(member => member.roles.cache.has(`1171473870616809555`)).forEach(member => {			
